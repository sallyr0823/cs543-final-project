...
class FaceBlender:
    ...
    def alpha_blend_images(self, source_image, target_image,alpha_1, alpha_2, source_tuple, target_tuple, skin_only=False):
        ...
        Args:
            source_image: Original image to be morphed
        ...
        morphed_image = np.copy(source_image)
                ...
                for y in range(min_y, max_y + 1):
                    ...
                    if source_triangulation.find_simplex(point) == idx:
                        ...
                        if skin_only and source_mask[y][x] == 2:  # Special handling for lips
                            morphed_image[y, x] = target_image[target_point[1], target_point[0]]
                        else:
                            morphed_image[y, x] = (
...
def main():
    ...
    source_tuple = FeatureDetection.landmark_detection(source_image)
    target_tuple = FeatureDetection.landmark_detection(target_image)
    
    alpha1 = 0.2  # Weight for source image
    alpha2 = 0.8  # Weight for target image
    morphed_image = blender.morph_images(
        source_image, 
        target_image,
        alpha1,
        alpha2,
    ...
    # Display results
    result = np.hstack((source_image, morphed_image,target_image))
    cv2.imshow("Face Morphing Result", result)
    cv2.waitKey(0)
