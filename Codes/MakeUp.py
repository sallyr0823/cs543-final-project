...
class MakeupTransfer:
    ...
    def process_image(self, image: np.ndarray) -> tuple:
        ...
        output_image, feature_points, triangulation, K = (
            FeatureDetection.landmark_detection(image)
        )
        lab_image = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
        lightness = lab_image[:, :, 0]
        color = lab_image[:, :, 1:3]
        structure_layer, texture_layer = wsFilter.wls_filter(lightness, K)
        
        return (output_image, feature_points, triangulation, K, 
    ...
    def transfer_makeup(self, source_image: np.ndarray, makeup_image: np.ndarray) -> np.ndarray:
        ...
        (source_output, source_features, source_tri, source_K,
         source_light, source_color, source_struct, source_texture) = self.process_image(source_image)
        
        (makeup_output, makeup_features, makeup_tri, makeup_K,
         makeup_light, makeup_color, makeup_struct, makeup_texture) = self.process_image(makeup_image)
        
        # Create tuples for blending
        source_tuple = (source_output, source_features, source_tri, source_K)
        makeup_tuple = (makeup_output, makeup_features, makeup_tri, makeup_K)
