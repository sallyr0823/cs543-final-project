...
class FaceLandmarkDetector:
    ...
    def landmark_detection(self,image):
        ...
        for triangle in triangles:
            ...
            cv2.line(ori_image, (feature_points[triangle[1]][0], feature_points[triangle[1]][1]),
                    (feature_points[triangle[2]][0], feature_points[triangle[2]][1]), (0, 0, 0))

        cv2.rectangle(ori_image, (minX, minY), (maxX, maxY), (255, 0, 0), 2)
        return ori_image,feature_points,triangulation,mask
...
def smooth_mask(mask, height, width):
    ...
    smoothed_mask = gaussian_filter(smoothed_mask, sigma=0.5)
...
def wls_filter(image_orig, beta):
    ...
    for i in range(0, beta.shape[0]- 1):
        for j in range(0, beta.shape[1] - 1):
            dx[i][j] = dx[i][j] * beta[i][j]
            dy[i][j] = dy[i][j] * beta[i][j] 



    dx = -lambda_ / (np.absolute(dx) ** alpha + eps)
    dy = -lambda_ / (np.absolute(dy) ** alpha + eps)
    dy = np.vstack((dy, np.zeros((1, s[1]))))
    dy = dy.flatten('F')

    dx = np.hstack((dx, np.zeros((s[0], 1))))
    dx = dx.flatten('F')
    A = spdiags(np.vstack((dx, dy)), [-s[0], -1], k, k)
    d = 1 - (dx + np.roll(dx, s[0]) + dy + np.roll(dy, 1))
    A = A + A.T
    A += spdiags(d, 0, k, k)
    res = spsolve(A, image.flatten('F')).reshape(s[::-1])
